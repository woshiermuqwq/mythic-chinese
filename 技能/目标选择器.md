...
| ------------------------------------- | ------- | ------------------------------------------------- |
| @Self                                 | @Caster | 选取施法者自身                                        |
| @SelfEyeLocation{length=5}（4.13） | @eyedirection | 选取施法者视角正前方第5格方块位置 |
| @Target                               | @T      | 选取自身的当前目标                                        |
| @Trigger                              |         | 选取激活触发器的实体                                  |
...
| @EntitiesInRadius{r=半径;types=种类,种类,种类}   | @EIR{r=半径;t=种类,种类,种类}  | 选取指定半径 球体范围内的全部实体 (包括创造/观察模式玩家)（从4.13开始支持变量与占位符） |
...
| @ItemsInRadius{r=半径}（4.13） | | 以坐标原点为中心,选取指定半径 球体范围内的所有掉落物 |
| @ItemsNearOrigin{r=半径}（4.13）             | @IIO{r=半径} / @INO{r=半径}| 以坐标原点为中心,选取指定半径 球体范围内的所有实体 |
...
| @Forward{forward(f)=5;yoffset(y)=0.0;sideOffset=0.0;rotation=0.0}    |           | 选取 当前朝向 正前方第5格方块位置且垂直偏移0.0格方块, 左右偏移0.0格位置的方块                                                                                                                                                                        |
| @ForwardWall{forward(f)=5;yoffset(y)=0.0;height(h)=3;width(w)=1}（4.13） | | 选取 当前朝向 正前方第5格方块位置且垂直偏移0.0格方块, 左右偏移0.0格位置 指定高度 宽度的一面墙 |
