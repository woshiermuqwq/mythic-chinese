...
| onTick              | oT       | 环绕到达下一个点时所激活的技能组<br>[坐标原点](/目标选择器/origin)为激活该技能组时时环绕所处点的位置<br>若技能组目标选择器为空则选取拥有该环绕的事物 | 无          |
| onHit               | oH       | 环绕抛射物命中实体后所激活的技能组<br>[坐标原点](/目标选择器/origin)为命中实体时环绕所处点的位置<br>若技能组目标选择器为空则选取拥有该环绕的事物 | 无          |
| onEnd               | oE       | 环绕结束所激活的技能组<br>[坐标原点](/目标选择器/origin)为结束时环绕所处点的位置<br>若技能组目标选择器为空则选取拥有该环绕的事物<br>拥有环绕的实体死亡并不会激活该选项 | 无          |
