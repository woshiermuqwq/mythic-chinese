...
<img src="https://s1.328888.xyz/2022/04/01/LXz23.png" alt="LXz23.png" border="0" width="500"/>
...
* 内容格式 必须为 Yaml（缩进等）
* 当存在俩个内部名称一致的实体时, 若其同处于一个文档内则只会读取位于上方的实体配置  
  若不处在同一文档内则必定有一个无法被读取
...
Mythic实体内部名:
  ...
  Trades:
```
![9999X.png](https://s1.328888.xyz/2022/03/18/9999X.png)
...
* 支持计算, 格式: `Damage: 数值 ADD(加法, 此选项放空则为乘以) 加上/乘以的数值`（5.2.1）
* 例子:
  * **Health: 200**
