...
| \@EntitiesNear[Origin](/技能/目标选择器/origin){r=半径} | \@LivingNear[Origin](/技能/目标选择器/origin){r=5} / \@EI[O](/技能/目标选择器/origin){r=5} / \@EN[O](/技能/目标选择器/origin){r=5}| 以[坐标原点](/技能/目标选择器/origin)为中心<br>选取指定半径<br>球体范围内的所有实体 |
...
| \@Forward{forward(f, amount, a)=5;yoffset(y)=0.0;<br>sideOffset=0.0;rotation=0.0;useeyedirection(uel)=false;lockpitch()=false(5.0.3)} | | 选取 当前朝向 正前方第5格方块位置且垂直偏移0.0格方块<br>左右偏移0.0格位置的方块<br>且不以眼睛前方作为选择基准<br>以视角作为基准而不是朝向(朝向不受俯仰视影响) |
...
| \@RLNT{amount=选取点数;maxradius=最大半径;spacing=每点间隔;minradius=最小半径} | \@RandomLocationsNearTarget{a=5;r=5;s=0;minr=0}<br>\@RandomLocationsNearTargetEntities<br>\@RandomLocationsNearTargetLocations<br>\@RandomLocationsNearTargets<br>\@RLNTE/\@RLNTL | 以技能目标为中心<br>随机选取方形范围内<br>指定数量的坐标点<br>作为技能目标<br>方形范围边长为`r`的值 |
| \@RLNO{amount=选取点数;maxradius=最大半径;spacing=每点间隔;minradius=最小半径} | \@RandomLocationsNear[Origin](/技能/目标选择器/origin){a=5;r=5;s=0;minr=0}<br>\@RandomLocations[Origin](/技能/目标选择器/origin)<<br>\@RL[O](/技能/目标选择器/origin) | 以[坐标原点](/技能/目标选择器/origin)为中心<br>指定数量的坐标点<br>作为技能目标<br>方形范围边长为`r`的值 |
| \@RLNC{amount=选取点数;maxradius=最大半径;spacing=每点间隔;minradius=最小半径} | \@RandomLocationsNearCaster{a=5;r=5;s=0;minr=0}<br>\@RandomLocations | 以施法者为中心<br>指定数量的坐标点<br>作为技能目标<br>方形范围边长为`r`的值 |
