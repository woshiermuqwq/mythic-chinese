...
掉落表则则位于DropTables文件夹内的任意常见格式(.txt .yml etc)的文档内.
...
| **掉落类型**           | **描述**                                          | **示例**                             |
|-----------------------------|----------------------------------------------------------|-----------------------------------------|
| **champions-exp**           | 给予插件: *Champions* 的经验  |                                         |
| **skillapi-exp**            | 给予插件: *SkillAPI* 的经验   |                                         |
| **heroesexp**               | 给予插件: *Heroes* 的经验  |                                         |
| **mcmmo-exp**               | 给予插件: *McMMO* 的经验  |                                         |
| **exp**                     | 给予原版经验  |                                         |
| **money**                   | 给予金钱 |                                         |
| **mythicdrop &lt;item&gt;** | 掉落 *MythicMobs* 物品 |                                         |
...
| **mythicmob**(4.14.0)                | 掉落 *MMOItems* 物品  | `- mmoitems{type=Mob;id=CUTLASS} 1 1` |
[更多关于MMOItems的信息,清点这儿](https://git.lumine.io/mythiccraft/mmoitems/-/wikis/Item%20Drop%20Tables)
...
| 选项名 | 值类型 | 描述 | 注意事项 |
| ------ | ------ | ---- | -------- |
| TotalItems | 数值 | 每行掉落数量被乘以的数值 | 无 |
| MaxItems | 数值 | 每行掉落数量被乘以的最小数值 | 无 |
| MinItems | 数值 | 每行掉落数量被乘以的最大数值 | 与MaxItems同时启用时,掉落表内每行掉落的几率会变成 weights |
