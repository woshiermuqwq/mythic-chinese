...
技能组被另一个技能组激活时, 该技能组的目标选择器及TargetConditions检测对象为 用于激活该技能组的技能行 所写的目标选择器

实体配置内的技能行无目标选择器时, 默认使用 @self

目标类条件(TargetConditions)所检测的对象被目标选择器所影响,如@self为检测自己,那样与Conditions无区别.
...
| 目标选择器名                          | 缩写    | 注释                                              |
|--------------------------------------|--------------------|----------------------------------------------------------------------------------------------------------|
| @LivingInCone{a=90.0;r=16.0;rot=0.0} | @EIC{a=弧度;}             | 扇形区域内的所有实体,详细见下 |
| @LivingInWorld                       | @EIW               | 所处世界的全部实体                                                             |
| @LivingEntitiesInRadius{r=半径}         |                    | 半径内的全部实体(包括创造/观察模式玩家) |
| @PlayersInRadius{r=半径}                | @PIR{r=半径}          | 半径内的全部玩家(不包括创造/观察模式玩家) |
| @MobsInRadius{r=半径;types=种类,种类,种类}       | @MIR{r=半径;t=种类,种类,种类}  | 选取半径内所有的指定实体(可不指定 可为MythicMobs实体) |
| @EntitiesInRadius{r=半径;types=种类,种类,种类}   | @EIR{r=半径;t=种类,种类,种类}  | 半径内的全部实体 (包括创造/观察模式玩家) |
| @PlayersInWorld                      | @World             | 所处世界的全部玩家 |
| @PlayersOnServer                     | @Server            | 服务器内的全部玩家 |
| @PlayersInRing{min=最小半径;max=最大半径}          |                    | 圆环内的全部玩家 |
| @PlayersNearOrigin{r=半径}              |                    | 以坐标原点为中心,选取半径范围内的所有玩家 |
| @MobsNearOrigin{r=#;t=种类,种类,种类}             |  | 以坐标原点为中心,选取半径范围内的所有生物(可不指定 可为MythicMobs实体)                                                                                                         |
| @EntitiesNearOrigin{r=半径}             | @EIO{r=半径} / @ENO{r=半径}| 以坐标原点为中心,选取半径范围内的所有实体 |
| @PlayersNearTargetLocation{r=半径}      | @PNTL{r=半径}         | 以技能目标位置为中心,选取半径范围内的所有玩家 |
