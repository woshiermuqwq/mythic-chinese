...
| ------------------------------------- | ------- | ------------------------------------------------- |
| @Self                                 | @Caster | 选取施法者自身                                        |
| @SelfEyeLocation{length=5}（5.0） | @eyedirection | 选取施法者视角正前方第5格方块位置 |
| @Target                               | @T      | 选取自身的当前目标                                        |
| @Trigger                              |         | 选取激活触发器的实体                                  |
...
| @UniqueIdentifier{u="<target.uuid>"}（5.0）  | @UUID   | UUID为指定字符串的实体,支持变量        |
| @Vehicle（4.12）                              |         | 施法者的坐                         |
...
| 目标选择器名                          | 缩写    | 注释                                              |
|--------------------------------------|--------------------|----------------------------------------------------------------------------------------------------------|
| @TargetedTarget（5.0） | @Targeted | 选取已被目标选择器所选取的实体 |
...
| @PlayersInRadius{r=半径}                | @PIR{r=半径}          | 选取指定半径 球体范围内的全部玩家(不包括创造/观察模式玩家)（从5.0开始支持变量与占位符） |
| @MobsInRadius{r=半径;types=种类,种类,种类}       | @MIR{r=半径;t=种类,种类,种类}  | 选取指定半径 球体范围内所有的指定实体(可不指定 可为MythicMobs实体) |
| @EntitiesInRadius{r=半径;types=种类,种类,种类}   | @EIR{r=半径;t=种类,种类,种类}  | 选取指定半径 球体范围内的全部实体 (包括创造/观察模式玩家)（从5.0开始支持变量与占位符） |
...
| @ItemsInRadius{r=半径}（5.0） | | 以坐标原点为中心,选取指定半径 球体范围内的所有掉落物 |
| @ItemsNearOrigin{r=半径}（5.0）             | @IIO{r=半径} / @INO{r=半径}| 以坐标原点为中心,选取指定半径 球体范围内的所有实体 |
...
| 目标选择器名        | 缩写      | 注释 
|------------------------|-----------|-------------------------------------------------------------------------------------------------------------- |
| @SelfLocation{y=0.0}          |           | 选取 自身位置且垂直偏移0.0格方块                                                                                                                                                                                                                                                                                                 |
| @Forward{f=5;y=0.0;sideOffset=0.0;rotation=0.0}    |           | 选取 当前朝向 正前方第5格方块位置且垂直偏移0.0格方块, 左右偏移0.0格方块                                                                                                                                                                        |
| @ForwardWall（5.0） | | 选取 当前朝向正前方的墙 |
...
| @ObstructingBlock（5.0）    |             | 选取与技能目标之间路径上的所有方块 |
| @NearestStructure（5.0） | | 选取离自身最近的结构（地牢等）的中心位置 |
| @CasterSpawnLocation（5.0）                  |         | 选取施法者的出生点                                      |
| @SpawnLocation（5.0）                  |         | 选取所处世界的世界出生点                                      |
...
| @Ring{radius=半径;points=点数;yoffset=0.0}              |            | 选取以自身为中心 指定半径 的环上的指定点数的位置 （从5.0开始支持变量与占位符）                                                                                           |
| @Sphere{radius=半径;points=点数;yoffset=0.0}              |            | 选取以自身为中心 指定半径 的球体内的的指定点数的位置 （从5.0开始支持变量与占位符）                                                                                           |
...
| @FloorOfTargets（5.0）     | @FOT       | 以目标脚下的方块为目标 |
| @LocationsOfTargets | @LOT       | 以技能目标的位置为目标        |
| @BlocksInRadius{radius=#;radiusy=#;noise=#;shape=选取范围的形状, 默认sphere sphere/cube;onlyair=是否仅选取空气false;noair=true}（5.0） |     | 选取半径范围内的所有方块 |
