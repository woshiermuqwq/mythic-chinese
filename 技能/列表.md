技能列表
===============

技能可写于技能配置内(技能配置位于实体配置内)  
技能也可写于技能组配置内(技能组配置位于Skills文件夹内)  
少数技能无需[目标选择器](/技能/目标选择器).

---------

少数技能只能选取位置为技能目标

| 技能名               | 描述                                                    |
|------------------------|----------------------------------------------------------------|
| [Activate Spawner][]   | 激活指定生成点(组)        |
| [Arrow Volley][]       | 瞬间发射多发箭矢                                       |
| [Aura Remove][]         | 移除目标身上的光环                         |
| [Bar Create][]          | 为目标创建Boss血条                   |
| [Bar Set][]             | 为目标修改Boss血条                  |
| [Bar Remove][]          | 为目标移除Boss血条                   |
| [Break Block][]         | 破坏指定位置的方块                        |
| [Break Block And Give Item][] | 破坏指定位置的方块后给予额外物品 |
| [Close Inventory][]    | 为目标(玩家)关闭背包                           |
| [Command][]            | 执行命令                            |
| [Consume][]            | 对目标造成伤害后恢复自身生命                |
| [Consume Slot][]            | 清除技能目标（玩家）指定槽位上指定数量的物品                |
| [Clear Threat][]        | 清除所有对目标拥有威胁度的实体                             |
| [Cquip][]                | 穿戴物品（仅可为Mythic物品）                                     |
| [Currency Give][]      | 给予金钱   |
| [Currency Take][]      | 夺取金钱 |
| [Disengage][]          | 向后冲刺                               |
| [Disguise][]           | 令自身伪装                                  |
| [Disguise Old][]     | 令自身伪装                                  |
| [Disguise Modify][]         | 修改目标的伪装                                   |
| [Disguise Modify New][]         | 修改目标的伪装                                   |
| [Disguise As Block][]     | 令目标伪装成方块                                  |
| [Disguise Target][]     | 令目标伪装                                  |
| [Disguise Remove][]         | 取消目标的伪装                                   |
| [Dismount][]           | 取消骑乘              |
| [Damage][]             | 对目标造成伤害(具体数值)                               |
| [Base Damage][]         | 对目标造成伤害(等值于武器攻击力百分比)      |
| [Damage Percent][]     | 对目标造成伤害(目标最大生命百分比)               |
| [Decapitate][]         | 掉落目标(玩家)头颅                       |
| [Doppleganger][]       | 令自身伪装成目标(玩家)                     |
| [Drop Item][]           | 在指定位置生成掉落物              |
| [Eject Passenger][]    | 取消被骑乘                              |
| [Equip][]                | 穿戴物品(可为原版物品)                      |
| [Explosion][]            | 生成爆炸                                                         |
| [Extinguish][]           | 熄灭目标身上的火                                   |
| [Fawe Paste][]            | 粘贴1个Schematic文件                                               |
| [Feed][]                 | 给予玩家饱和度                                                     |
| [Fill Chest][]            | 用物品与掉落表填充箱子                                    |
| [Fly][]                  | 给予目标一个允许飞行的[光环](技能/列表/Aura)                           |
| [Force Pull][]           | 将目标传送到自身位置                                             |
| [Freeze][]               | 将目标冻结一段时间（有霜冻屏幕效果 需1.17).                |
| [Glow][]                 | 令目标发光                                                       |
| [Give Item][]            | 给予目标物品（目标背包无空位时无效）                                                |
| [Give Item From Slot][] | 给予目标 位于自身指定槽位的物品（目标背包无空位时无效） |
| [Give Item From Target][] | 夺取目标物品并给予给施法者                                |
| [Go to][]             | 强制施法者（非玩家）走向技能目标                               |
| [Heal][]                 | 恢复目标生命(具体数值)                                                            |
| [Heal Percent][]          | 恢复目标生命(目标最大生命百分比)                         |
| [Hide From Players][] | 令所有玩家在指定时间内无法看见施法者（须为玩家） |
| [Hologram][] | 在技能目标位置召唤悬浮字实体　|
| [Ignite][]               | 点燃目标                                                     |
| [JSON Message][]         | 向目标(玩家)发送Json信息                         |
| [Jump][]                 | 跳向目标                                                   |
| [Leap][]                 | 向目标冲刺                                |
| [Lightning][]            | 在目标位置生成雷击                                             |
| [Look][]                 | 看向目标                                     |
| [Lunge][]                | 朝目标冲刺（提供上下速度）                            |
| [Message][]              | 向目标(玩家)发送聊天栏信息                                     |
| [Modify Global Score][]  | 修改虚拟目标: \_\_GLOBAL\_\_ 在指定记分板上的分数                                          |
| [Modify Target Score][]  | 修改目标在指定记分板上的分数                                   |
| [Modify Score][]         | 修改虚拟目标在指定记分板上的分数                                       |
| [Mount][]                | 生成MythicMobs实体并骑乘                                  |
| [Mount Me][]             | 令目标骑上施法者                                |
| [Mount Target][]         | 令施法者骑上目标                                                           |
| [Oxygen][]               | 给予目标(玩家)氧气                                             |
| [Play Animation][] | 令技能目标播放指定动作（≥1.17） |
| [Play Block Break Sound][] | 播放目标位置的方块的破坏音效 |
| [Play Block Hit Sound][] | 播放目标位置的方块的挖掘音效 |
| [Play Block Place Sound][] | 播放目标位置的方块的放置音效 |
| [Play Block Step Sound][] | 播放目标位置的方块的踩踏音效 |
| [Pose ArmorStand][]       | 修改盔甲架姿势                                   |
| [Potion][]               | 将状态效果应用于目标                                       |
| [Potion Clear][]          | 清除目标全部的状态效果                               |
| [Prison][]               | 令目标困在由方块组成的空间内                                         |
| [Propel][]           | 朝目标冲刺（提供上下速度）                                  |
| [Pick Up Item][]           | 拾取技能目标（掉落物实体）                                  |
| [Pull][]                 | 强制目标往施法者位置移动                                            |
| [Push Button][]           | 按下指定位置的按钮                                      |
| [Rally][]                | 令范围内的所有指定实体选取目标为目标                               |
| [Random Message][]       | 随机发送多条聊天栏信息中的一条给目标(玩家)                                 |
| [Remount][]              | 重新骑上坐骑(若坐骑存活) |
| [Remove][]               | 移除目标(非玩家)                                                      |
| [Remove Held Item][]       | 移除目标的手持物品                       |
| [Remove Owner][]          | 令狼忘记主人                                     |
| [Run AI Goal Selector][] | 修改目标的AI行动器                                                    |
| [Run AI Target Selector][] | 修改目标的AI目标选择器                                              |
| [Send Action Message][]    | 发送活动栏信息给目标(玩家)                           |
| [Send Resource Pack][]     | 发送资源包下载链接给目标(玩家)                                |
| [Send Title Message][]     | 发送标题信息给目标(玩家)                       |
| [Send Toast][]             | 发送进度信息给目标(玩家)                           |
| [Set AI][]                 | 开关目标(非玩家)的AI                                 |
| [Set Block Type][]         | 修改指定位置的方块种类                                      |
| [Set Game Mode][]          | 修改目标(玩家)的游戏模式                                   |
| [Set Gliding][]            | 设置目标身上的鞘翅飞行状态                                |
| [Set Global Score][]       | 设置虚拟玩家: \_\_GLOBAL\_\_ 在指定记分板的分数                                            |
| [Set Gravity][]            | 设置目标(非玩家)是否拥有重力                            |
| [Set Health][]             | 设置目标(非玩家)的当前生命                                      |
| [Set Level][]              | 设置MythicMobs实体的等级                                           |
| [Set Max Health][]         | 设置目标(非玩家)的最大生命                                  |
| [Set Mob Color][]          | 设置目标(特定实体)的外观颜色                  |
| [Set Mob Score][]          | 设置目标在指定记分板上的分数                                |
| [Set Name][]               | 设置目标(非玩家)的显示名                                          |
| [Set No Damage Ticks][]      | 设置目标的受伤间隔                                      |
| [Set Owner][]              | 设置目标为施法者的主人                             |
| [Set Rotation][]           | 设置目标朝向（不能为玩家）                                           |
| [Set Target Score][]       | 设置目标在指定记分板上的分数                                              |
| [Set Score][]              | 设置虚拟玩家在指定记分板上的分数                               |
| [Set Speed][]              | 设置目标在指定移动状态下的移动速度                                  |
| [Set Stance][]             | 设置目标的当前姿势                                         |
| [Shield][]                 | 为目标提供黄心                             |
| [Shield Percent][]          | 为目标提供黄心(目标最大生命百分比) |
| [Shoot Fireball][]         | 向目标发射火球                                           |
| [Shoot Potion][]           | 向目标投掷药水瓶                                             |
| [Shoot Skull][]            | 向目标发射凋灵头颅                                      |
| [Shoot Shulker Bullet][]   | 向目标发射潜影贝导弹                              |
| [Signal][]                 | 向目标发送信号                                                   |
| [Speak][]                  | 发送聊天栏信息并以悬浮字形式显示文本         |
| [Spring][]                 | 在目标脚下生成临时液体方块                        |
| [Stun][]                   | 晕眩目标                                                   |
| [Suicide][]      | 令施法者自杀                                                            |
| [Summon][]       | 在目标位置生成实体                                                    |
| [Swap][]         | 与目标互换位置                                                     |
| [Add Tag][]      | 为目标添上标签                                                 |
| [Remove Tag][]   | 为移除添上标签                                             |
| [Teleport][]     | 传送到指定目标                                                             |
| [TeleportIn][]   | 将指定目标传送到自身位置,可调整位置偏移.                                   |
| [TeleportTo][]   | 将目标传送到指定位置                                        |
| [Threat][]       | 修改目标对于施法者的威胁度                                        |
| [Throw][]        | 投掷目标                                                            |
| [Toggle Lever][] | 开关指定位置的拉杆                                              |
| [Toggle Sitting][] | 令目标(特定实体)坐下/站起                      |
| [Track Location][] | 记录目标的当前位置
| [Velocity][]     | 设置目标的当前动量                                       |
| [Weather][]      | 修改目标所处世界的天气                                           |
| [WolfSit][]      | 令目标(狗)坐下                                                 |

特效技能
----------------

特效技能拥有自己的页面!

[点击这里](技能/特效列表)

特殊技能
-----------------------

一些目标选择器作用于特殊技能,效果将有所变化(如@origin).  
特殊技能几乎全都可以调用其它技能组.  
有许多其它技能没有的模块.

| 技能名            | 描述                                                                             |
|---------------------|-----------------------------------------------------------------------------------------|
| **[Skill][]**       | 向目标激活指定技能组                                       |
| [Aura][]            | 给予目标光环 |
| [Cancel Event][]     | 取消对应的触发器事件                                  |
| [Cast][]            | 给予施法者用于激活多个技能组的光环 |
| [Chain][]           | 在多个目标之间传递指定技能组                       |
| [Chain Missile][]    | 在多个目标之间传递导弹(Missile)                |
| [Delay][]           | 延迟多少刻后激活技能行                                              |
| [Global Cooldown][] | 设置施法者所有技能的冷却时间                                                 |
| [Missile][]         | 向目标发射追踪抛射物                                                       |
| [Modify Projectile][] | 修改已发射的抛射/弹射物与环绕                                   |
| [On Attack][]        | 给予技能目标光环(光环持有者近身攻击后激活技能组)                 |
| [On Block Break][]       | 给予技能目标光环(光环持有者成功挖掘方块后激活技能组)            |
| [On Block Place][]       | 给予技能目标光环(光环持有者放置方块后激活技能组)            |
| [On Damaged][]       | 给予技能目标[光环](技能/列表/Aura)(光环持有者受到任何形式的伤害后激活技能组)            |
| [On Interact][]         | 给予技能目标光环(光环持有者被交互后激活技能组)            |
| [On Jump][]        | 给予技能目标光环(光环持有者跳跃后激活技能组)                 |
| [On Death][]       | 给予技能目标[光环](技能/列表/Aura)(光环持有者死亡后激活技能组)            |
| [On Shoot][]         | 给予技能目标光环(光环持有者射击后激活技能组)            |
| [On Swing][]         | 给予技能目标光环(光环持有者左键后激活技能组)            |
| [Orbital][]         | 生成抛射物环环绕技能目标                 |
| [Projectile][]      | 向技能目标发射抛射物                               |
| [Shoot][]           | 向技能目标发射弹射物                                                  |
| [Volley][]            | 从施法者位置向目标发射指定数量的弹射物 |
| [Ray Trace][]         | 从施法者位置向目标发射射线            |
| [Sudo Skill][]         | 令目标作为施法者激活技能                                       |
| [Random Skill][]      | 向目标激活多个技能组内的一个                                    |
| [Totem][]             | 图腾       |
| [Variable Add][]      | 增加变量值                                   |
| [Variable Math][]     | 计算变量值                                    |
| [Set Variable][]      | 创建变量(若存在则替换)                                           |
| [Variable Unset][]    | 移除变量                                                   |
| [Variable Subtract][] | 减少变量值                            |

通用修改项
--------------------

这些修改项可用于所有技能,且全部支持[变量](技能/变量)  
20tick=1秒.

| 修改项名      | 缩写 | 描述                                    | 默认值 |
|----------------|-----------|------------------------------------------------|---------|
| [cooldown](技能/列表/cooldown)       | cd        | 冷却时间(秒)                                     | 0       |
| [delay](技能/列表/delay)          |           | 延迟(刻)           | 0       |
| [repeat](技能/列表/repeat)         |           | 重复激活次数 | 0       |
| [repeatInterval](技能/列表/repeat) |           | 重复激活间隔(刻) | 0       |

即将到来
------------------

| 技能名           | 描述                    |
|--------------------|---------------------------------|
| [Time][]           | 修改目标所处世界的时间                |


  [here!]: /skills/skillparametersystem
  [Targeter]: /skills/targeters/
  [Activate Spawner]: /技能/列表/activatespawner
  [Animate ArmorStand]: /技能/列表/animatearmorstand
  [Arrow Volley]: /技能/列表/arrowvolley
  [Aura Remove]: /技能/列表/auraremove
  [Bar Create]: /技能/列表/barcreate
  [Bar Set]: /技能/列表/barset
  [Bar Remove]: /技能/列表/barremove
  [Break Block]: /技能/列表/breakblock
  [Break Block And Give Item]: /技能/列表/breakBlockAndGiveItem
  [Close Inventory]: /技能/列表/closeinventory
  [Command]: /技能/列表/command
  [Consume]: /技能/列表/consume
  [Consume Slot]: /技能/列表/consumeslot
  [Disengage]: /技能/列表/disengage
  [Disguise]: /技能/列表/disguise
  [Disguise Old]: /技能/列表/disguiseold
  [Disguise As Block]: /技能/列表/disguiseasblock
  [Disguise Target]: /技能/列表/disguisetarget
  [Disguise Modify]: /技能/列表/disguisemodify
  [Disguise Modify New]: /技能/列表/disguisemodifynew
  [Disguise Remove]: /技能/列表/undisguise
  [Dismount]: /技能/列表/dismount
  [Clear Threat]: /技能/列表/clearthreat
  [Cquip]: /技能/列表/cquip
  [Currency Give]: /技能/列表/currencygive
  [Currency Take]: /技能/列表/currencytake
  [Damage]: /技能/列表/damage
  [Base Damage]: /技能/列表/basedamage
  [Damage Percent]: /技能/列表/damagepercent
  [Decapitate]: /技能/列表/decapitate
  [Doppleganger]: /技能/列表/doppleganger
  [Drop Item]: /技能/列表/dropitem
  [Eject Passenger]: /技能/列表/ejectpassenger
  [Equip]: /技能/列表/equip
  [Explosion]: /技能/列表/explosion
  [Extinguish]: /技能/列表/extinguish
  [Fawe Paste]: /fawePaste
  [Feed]: /技能/列表/feed
  [Fill Chest]: /技能/列表/fillChest
  [Fly]: /技能/列表/fly
  [Force Pull]: /技能/列表/forcepull
  [Glow]: /技能/列表/glow
  [Give Item]: /技能/列表/giveitem
  [Give Item From Target]: /技能/列表/giveitemfromtarget
  [Go To]: /技能/列表/goto
  [Heal]: /技能/列表/heal
  [Heal Percent]: /技能/列表/healpercent
  [Ignite]: /技能/列表/ignite
  [JSON Message]: /技能/列表/jsonmessage
  [Jump]: /技能/列表/jump
  [Leap]: /技能/列表/leap
  [Lightning]: /技能/列表/lightning
  [Look]: /技能/列表/look
  [Lunge]: /技能/列表/lunge
  [Message]: /技能/列表/message
  [Modify Global Score]: /技能/列表/modifyglobalscore
  [Modify Target Score]: /技能/列表/modifytargetscore
  [Modify Score]: /技能/列表/modifyscore
  [Mount]: /技能/列表/mount
  [Mount Me]: /技能/列表/mountme
  [Mount Target]: /技能/列表/mounttarget
  [Oxygen]: /技能/列表/oxygen
  [Play Animation]: /技能/列表/playanimation
  [Pick Up Item]: /技能/列表/pickupitem
  [Play Block Break Sound]: /技能/列表/playblockbreaksound
  [Play Block Hit Sound]: /技能/列表/playblockhitsound
  [Play Block Place Sound]: /技能/列表/playblockplacesound
  [Play Block Step Sound]: /技能/列表/playblockstepsound
  [Pose ArmorStand]: /技能/列表/posearmorstand
  [Potion]: /技能/列表/potion
  [Potion Clear]: /技能/列表/potionclear
  [Prison]: /技能/列表/prison
  [Propel]: /技能/列表/propel
  [Pull]: /技能/列表/pull
  [Push Button]: /技能/列表/pushbutton
  [Rally]: /技能/列表/rally
  [Random Message]: /技能/列表/randommessage
  [Ray Trace]: /技能/列表/raytrace
  [Ray Trace To]: /技能/列表/raytraceto
  [Remount]: /技能/列表/remount
  [Remove]: /技能/列表/remove
  [Remove Held Item]: /技能/列表/removehelditem
  [Remove Owner]: /技能/列表/removeowner
  [Run AI Goal Selector]: /技能/列表/runaigoalselector
  [Run AI Target Selector]: /技能/列表/runaitargetselector
  [Send Action Message]: /技能/列表/sendactionmessage
  [Send Resource Pack]: /技能/列表/sendresourcepack
  [Send Title Message]: /技能/列表/sendtitlemessage
  [Send Toast]: /技能/列表/sendtoast
  [Set AI]: /技能/列表/setai
  [Set Block Type]: /技能/列表/setblocktype
  [Set Game Mode]: /技能/列表/setgamemode
  [Set Gliding]: /技能/列表/setgliding
  [Set Global Score]: /技能/列表/setglobalscore
  [Set Gravity]: /技能/列表/setgravity
  [Set Health]: /技能/列表/sethealth
  [Set Level]: /技能/列表/setlevel
  [Set Max Health]: /技能/列表/setmaxhealth
  [Set Mob Color]: /技能/列表/setmobcolor
  [Set Mob Score]: /技能/列表/setmobscore
  [Set Name]: /技能/列表/setname
  [Set No Damage Ticks]: /技能/列表/setnodamageticks
  [Set Owner]: /技能/列表/setowner
  [Set Rotation]: /技能/列表/setrotation
  [Set Target Score]: /技能/列表/settargetscore
  [Set Score]: /技能/列表/setscore
  [Set Speed]: /技能/列表/setspeed
  [Set Stance]: /技能/列表/setstance
  [Shield]: /技能/列表/shield
  [Shield Percent]: /技能/列表/shieldpercent
  [Shoot Fireball]: /技能/列表/shootfireball
  [Shoot Potion]: /技能/列表/shootpotion
  [Shoot Skull]: /技能/列表/shootskull
  [Shoot Shulker Bullet]: /技能/列表/shootshulkerbullet
  [Signal]: /技能/列表/signal
  [Speak]: /技能/列表/speak
  [Spring]: /技能/列表/spring
  [Stun]: /技能/列表/stun
  [Suicide]: /技能/列表/suicide
  [Summon]: /技能/列表/summon
  [Add Tag]: /技能/列表/addtag
  [Remove Tag]: /技能/列表/removetag
  [Teleport]: /技能/列表/teleport
  [TeleportIn]: /技能/列表/teleportin
  [TeleportTo]: /技能/列表/teleportto
  [Threat]: /技能/列表/threat
  [Throw]: /技能/列表/throw
  [Toggle Lever]: /技能/列表/togglelever
  [Toggle Sitting]: /技能/列表/togglesitting
  [Velocity]: /技能/列表/velocity
  [Weather]: /技能/列表/weather
  [WolfSit]: /技能/列表/wolfsit
  [点击这里]: /技能/特效技能/
  [Skill]: /技能/列表/skill
  [Aura]: /技能/列表/aura
  [Cancel Event]: /技能/列表/cancelevent
  [Cast]: /技能/列表/cast
  [Chain]: /技能/列表/chain
  [Chain Missile]: /技能/列表/chainmissile
  [Delay]: /技能/列表/delay
  [Global Cooldown]: /技能/列表/globalcooldown
  [Missile]: /技能/列表/missile
  [Modify Projectile]: /技能/列表/modifyprojectile
  [On Attack]: /技能/列表/onattack
  [On Block Break]: /技能/列表/onblockbreak
  [On Block Place]: /技能/列表/onblockplace
  [On Damaged]: /技能/列表/ondamaged
  [On Death]: /技能/列表/ondeath
  [On Swing]: /技能/列表/onswing
  [On Interact]: /技能/列表/oninteract
  [On Jump]: /技能/列表/onjump
  [On Shoot]: /技能/列表/onshoot
  [Orbital]: /技能/列表/orbital
  [Projectile]: /技能/列表/projectile
  [Shoot]: /技能/列表/shoot
  [Volley]: /技能/列表/volley
  [Sudo Skill]: /技能/列表/sudoskill
  [Random Skill]: /技能/列表/randomskill
  [Totem]: /技能/列表/totem
  [Variable Add]: /技能/列表/variableadd
  [Variable Math]: /技能/列表/variablemath
  [Set Variable]: /技能/列表/setvariable
  [Variable Unset]: /技能/列表/variableunset
  [Variable Subtract]: /技能/列表/variablesubtract
  [Swap]: /技能/列表/swap
  [Time]: /技能/列表/time
  [Freeze]: /技能/列表/freeze
  [Hide From Players]: /技能/列表/hidefromplayers
  [Track Location]: /技能/列表/tracklocation
  [Give Item From Slot]: /技能/列表/giveitemfromslot
  [HoloGram]: /技能/列表/hologram