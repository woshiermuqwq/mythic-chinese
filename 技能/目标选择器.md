...
| \@EntitiesNear[Origin](/技能/目标选择器/origin){r=半径} | \@LivingNar[Origin](/技能/目标选择器/origin){r=5} / \@EI[O](/技能/目标选择器/origin){r=5} / \@EN[O](/技能/目标选择器/origin){r=5}| 以[坐标原点](/技能/目标选择器/origin)为中心<br>选取指定半径<br>球体范围内的所有实体 |
| \@ItemsInRadius{radius=5}（4.13） | \@IIR{r=半径} | 以施法者为中心<br>选取指定半径<br>球体范围内的所有掉落物 |
| \@ItemsNear[Origin](/技能/目标选择器/origin){radius=5}（4.13） | \@II[O](/技能/目标选择器/origin){r=半径} / \@IN[O](/技能/目标选择器/origin){r=半径}| 以[坐标原点](/技能/目标选择器/origin)为中心<br>选取指定半径<br>球体范围内的所有实体 |
...
| \@LivingEntitiesInRadius{livingonly=true;radius=5} | \@LivingEntitiesInRadius{livingonly=是否排除箭矢等非生物实体;radius=半径}<br>\@EntitiesInRadius<br>\@AllInRadius<br>\@EIR{l=true;r=5} | 以自身为中心<br>选取指定半径<br>球体范围内的全部实体(包括创造/观察模式玩家)<br>选项: livingonly 新增于 MM 4.13, 也就是说在这之前技能不可能选取箭矢等实体 |
...
这些目标选择器共有的修改项有:
* Xoffset/xo/x: X轴偏移(格方块)
* Yoffset/yo/y: Y轴偏移(格方块)
* Zoffset/zo/z: Z轴偏移(格方块)
* ForwardOffset/foffset/fo: 前后偏移(格方块 基于视角)
* SideOffset/soffset/so: 左右偏移(格方块 基于视角)
* Rotatex/rotx: X轴旋转
* Rotatey/roty: Y轴旋转
* Rotatez/rotz: Z轴旋转
* Coordinatex, cx: X轴坐标
...
| \@Forward{forward(f, amount, a)=5;yoffset(y)=0.0;sideOffset=0.0;rotation=0.0;useeyedirection(uel)=false;lockpitch()=false(5.0.3)} | | 选取 当前朝向 正前方第5格方块位置且垂直偏移0.0格方块<br>左右偏移0.0格位置的方块<br>且不以眼睛前方作为选择基准<br>以视角作为基准而不是朝向(朝向不受俯仰视影响) |
...
| \@[Origin](/技能/目标选择器/origin){xoffset=0;yoffset=0;zoffset=0;sideoffset（5.0）=1;forwardoffset（5.0）=2} | \@[Source](/技能/目标选择器/origin) | 选取坐标原点<br>并三轴偏移0格方块,前后偏移1格方块<br>左右偏移2格方块. 详细信息->[[1]](技能/目标选择器/Origin) |
...
| \@VariableLocation（5.0.3） | @VarLocation | 选取变量值所指定的位置 |
...
| \@RLNO{amount=选取点数;maxradius=最大半径;spacing=每点间隔;minradius=最小半径} | \@RandomLocationsNear[Origin](/技能/目标选择器/origin){a=5;r=5;s=0;minr=0}<br>\@RandomLocations[Origin](/技能/目标选择器/origin)<<br>\@RL[O](/技能/目标选择器/origin) | 以[坐标原点](/技能/目标选择器/origin)为中心<br>随机选取以指定半径所形成的球体范围内<br>指定数量的坐标点<br>作为技能目标 |
